  ...
  if (editing || !mySchedule) {
    return (
      <ScheduleForm
        ...
        myGroups={myGroups}
        onSave={async (slot1, slot2) => { await onSave(slot1, slot2); setEditing(false); }}
...
function ScheduleForm({ initial, userId, username, myGroups, onSave, onCancel }) {
  ...
  const [form2, setForm2] = useState({ daysOfWeek:[], startTime:"20:00", endTime:"22:00" });
  const [error, setError] = useState("");
  ...
  const dayBtn = (sel) => ({padding:"10px 0",borderRadius:"8px",fontSize:"11px",fontWeight:700,border:"none",cursor:"pointer",textAlign:"center",background:sel?"#fbbf24":"rgba(255,255,255,0.1)",color:sel?"#1c1400":"rgba(255,255,255,0.6)"});
  ...
  function save() {
    ...
    if (hasSecond && form2.daysOfWeek.length === 0) {
      setError("Please select days for your 2nd time slot, or remove it."); return;
    }
    setError("");

    // Pass BOTH slots in one object — parent handles all DB operations
    const slot1 = { ...(initial||{}), id:initial?.id||null, isExisting:!!initial?.id, userId, hostUsername:username, platform:form.platform.trim(), communityId:form.communityId, daysOfWeek:[...form.daysOfWeek].sort((a,b)=>a-b), startTime:form.startTime, endTime:form.endTime, notes:form.notes.trim(), manualStatus:initial?.manualStatus??null, createdAt:initial?.createdAt||new Date().toISOString() };
    const slot2 = hasSecond ? { ...slot1, id:null, isExisting:false, createdAt:new Date().toISOString(), daysOfWeek:[...form2.daysOfWeek].sort((a,b)=>a-b), startTime:form2.startTime, endTime:form2.endTime } : null;
      ...
      <div style={{background:"linear-gradient(145deg,#1e2340,#16192e)",borderRadius:"20px",padding:"20px",display:"flex",flexDirection:"column",gap:"14px"}}>
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"4px"}}>
            {DAYS.map((day,i) => <button key={i} type="button" onClick={()=>toggleDay(i)} style={dayBtn(form.daysOfWeek.includes(i))}>{day.slice(0,2)}</button>)}
          </div>
        </div>
      ...
      {!hasSecond ? (
        ...
        <div style={{background:"linear-gradient(145deg,#1e2340,#16192e)",borderRadius:"20px",padding:"20px",display:"flex",flexDirection:"column",gap:"14px",border:"1px solid rgba(251,191,36,0.2)"}}>
          ...
          <div>
            <label style={{...IS.label,marginBottom:"10px"}}>Days of Week * <span style={{color:"rgba(255,255,255,0.3)",fontWeight:400}}>(tap to select)</span></label>
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"4px"}}>
              {DAYS.map((day,i) => <button key={i} type="button" onClick={()=>toggleDay2(i)} style={dayBtn(form2.daysOfWeek.includes(i))}>{day.slice(0,2)}</button>)}
            </div>
          </div>
...
async function handleLogout() {
  ...
  // ── Schedules ──
  async function handleSaveSchedule(s, slot2 = null) {
    const payload = {
    ...
    // Save 2nd slot if provided — fresh INSERT across all communities
    if (slot2) {
      const communityIds = currentUser.communityIds || [];
      for (const cid of communityIds) {
        const { data: newRow, error: err2 } = await supabase
          .from("schedules")
          .insert({
            user_id:       slot2.userId,
            community_id:  cid,
            host_username: slot2.hostUsername,
            platform:      slot2.platform,
            days_of_week:  slot2.daysOfWeek,
            start_time:    slot2.startTime,
            end_time:      slot2.endTime,
            notes:         slot2.notes || "",
            manual_status: null,
          })
          .select("id, user_id, community_id, host_username, platform, days_of_week, start_time, end_time, notes, manual_status, created_at")
