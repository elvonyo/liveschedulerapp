 ...
 *  2. Calls your API to verify the session and mark the user as paid in Supabase
 *  3. Redirects back to the main app
 *
 * [DB INTEGRATION] The /api/stripe/verify route (below) handles the Supabase update.
 * In production, the Stripe webhook also handles this — this page is a fallback
 * for cases where the webhook hasn't fired yet by the time the user lands here.
 */
...
export default function PaymentSuccess() {
  ...
  useEffect(() => {
    ...
    // Verify the session with your backend and mark user as paid
    // [DB INTEGRATION] This calls /api/stripe/verify which updates profiles.has_paid = true
    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: "420px", textAlign: "center" }}>
        ...
        {status === "loading" && (
          <>
            <div style={{
              width: 48, height: 48, border: "3px solid rgba(251,191,36,0.3)",
              borderTopColor: "#fbbf24", borderRadius: "50%",
              animation: "spin 0.8s linear infinite",
              margin: "0 auto 20px",
            }} />
            <p style={{ color: "#fff", fontWeight: 900, fontSize: "20px", margin: "0 0 8px" }}>
        ...
        {status === "success" && (
          <>
            ...
            <p style={{ color: "#fff", fontWeight: 900, fontSize: "22px", margin: "0 0 8px" }}>
              You're in!
            </p>
            ...
            <div style={{
              background: "linear-gradient(145deg,#1e2340,#16192e)",
              borderRadius: "16px",
              padding: "16px",
              border: "1px solid rgba(52,211,153,0.2)",
            }}>
              <p style={{ color: "#6ee7b7", fontSize: "13px", margin: 0 }}>
                ✓ Welcome to LiveSupport Scheduler
              </p>
            </div>
          </>
        )}
        ...
        {status === "error" && (
          <>
            ...
            <button
              onClick={() => router.push("/")}
              style={{
                width: "100%", background: "#fbbf24", color: "#1c1400",
